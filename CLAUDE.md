...
- `[security]` and `[exchanges.*]` sections are generated by `poscli init`. **Never
  edit them by hand.** Touching the ciphertext, salt, or KDF params will brick
  decryption with no diagnostic. To change credentials use `poscli add <exchange>`;
  to change the master password use `poscli rotate-password` (when implemented).
- `[runtime]` is plain-text user preference (`use_testnet`, `http_timeout_sec`,
  `log_file`). Editing it by hand is fine — these are just toggles. Mode stays 0600;
  `Load` re-checks it on startup.
